(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(['module', './users', 'graphql-tag', 'axios', 'ethers', 'bignumber', 'apollo-link', '../utils/web3'], factory);
  } else if (typeof exports !== "undefined") {
    factory(module, require('./users'), require('graphql-tag'), require('axios'), require('ethers'), require('bignumber'), require('apollo-link'), require('../utils/web3'));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod, global.users, global.graphqlTag, global.axios, global.ethers, global.bignumber, global.apolloLink, global.web3);
    global.index = mod.exports;
  }
})(this, function (module, _users, gql, axios, ethers, BigNumber) {
  'use strict';
  ...
  const { getContract, getWeb3Provider };
    ...
    async allUsersWithHat(hatID) {
      ...
      if (res.data && res.data.accounts) {
        ...
        for (let i = 0; i < accounts.length; i++) {
          if (JSON.parse(accounts[i].balance) > topDonor.balance) topDonor = accounts[i];
        }
      }
    ...
    async getTopDonorByHatGroup(hats) {
      ...
      if (hatsArray && hatsArray.length) {
        for (let i = 0; i < hatsArray.length; i++) {
          const { topDonor } = await this.allUsersWithHat(hatsArray[i].toString());
    ...
    async sortHatsByReceivedSavingsOf(hats) {
      ...
      if (hatsArray && hatsArray.length) {
        for (let i = 0; i < hatsArray.length; i++) {
          ...
          const receivedSavingsOf = await this.receivedSavingsOfByHat(hatsArray[i]);
